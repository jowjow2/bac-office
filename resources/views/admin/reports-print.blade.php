...
<head>
    ...
    <title>BAC Office Reports</title>
    <style>
        body {
        ...
        <tbody>
            @foreach($projectSummary as $project)
                <tr>
                    ...
                    <td>{{ (float) $project->awarded_amount > 0 ? 'P' . number_format((float) $project->awarded_amount, 2) : '-' }}</td>
                    <td>{{ \Illuminate\Support\Str::headline($project->status) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
