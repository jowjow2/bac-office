...
<div class="admin-dashboard dashboard-home staff-dashboard-page bidder-messages-page">
    ...
    <div class="main-area">
        <header class="navbar">
            <div class="nav-left">
                <div>
                    <h2>Messages</h2>
                    <p>Chat with the BAC Office team about your account and bids.</p>
                </div>
            </div>
        ...
        <main class="dashboard-content dashboard-home-content">
            <section class="staff-messages-shell">
                ...
                @php
                    ...
                    $activeTitle = $activeTab === 'staff' ? 'Message Staff' : 'Message Admin';
                    $activeDescription = $activeTab === 'staff'
                        ? 'Select a staff contact and open a live chat.'
                        : 'Send questions and updates to the BAC admin team.';
                @endphp

                <nav class="staff-message-tabs" aria-label="Message categories">
                    <a href="{{ route('bidder.messages', ['tab' => 'admin']) }}" class="staff-message-tab {{ $activeTab === 'admin' ? 'is-active' : '' }}">
                        <i class="fas fa-user-shield"></i>
                        Admin
                    </a>
                    <a href="{{ route('bidder.messages', ['tab' => 'staff']) }}" class="staff-message-tab {{ $activeTab === 'staff' ? 'is-active' : '' }}">
                ...
                <section class="staff-messages-panel">
                    <div class="staff-messages-panel-header">
                        ...
                        <div class="staff-message-search">
                            ...
                            <input type="text" id="bidderMessageSearch" placeholder="Search conversations...">
                        </div>
                    </div>

                    <div class="staff-message-list" id="bidderMessageList">
                        @forelse($activeThreadSummaries as $thread)
                            @php
                                ...
                                $threadRoleLabel = $threadUser->role === 'staff' ? 'Staff' : 'Admin';
                                $threadRouteTab = $threadUser->role === 'staff' ? 'staff' : 'admin';
                                $messageAction = $threadUser->role === 'staff' ? 'Message Staff' : 'Message Admin';
                                ...
                                $isSelected = (int) ($selectedAdmin?->id ?? 0) === (int) $threadUser->id;
                            ...
                            @endphp
                            <a
                                href="{{ route('bidder.messages', ['tab' => $threadRouteTab, 'user' => $threadUser->id]) }}"
                        ...
                        @empty
                            <div class="staff-message-empty">
                                ...
                                No {{ $activeTab === 'staff' ? 'staff' : 'admin' }} conversations are available yet.
                            </div>
                        @endforelse
                    </div>
                </section>
...
@include('partials.message-chat-modal', [
    ...
    'messageChatEmptyTitle' => 'Select a BAC contact',
    'messageChatEmptyText' => 'Choose an admin or staff contact to open a live conversation.',
])

<script>
    document.addEventListener('DOMContentLoaded', function () {
        const searchInput = document.getElementById('bidderMessageSearch');
        const list = document.getElementById('bidderMessageList');
