            ...
            <div id="createModalOverlay" style="display: flex; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.92); z-index: 10000; justify-content: center; align-items: center; padding: 20px; box-sizing: border-box; backdrop-filter: blur(4px);">
                <div class="form-container" style="background: #111827; border-radius: 16px; width: 100%; max-width: 800px; max-height: calc(100vh - 40px); display: flex; flex-direction: column; position: relative; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); overflow: hidden; margin: 0; color: #ffffff;">
                    ...
                    <div style="padding: 20px 24px; border-bottom: 1px solid #374151; background: #111827; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h2 style="margin: 0; font-size: 20px; font-weight: 600; color: #ffffff;">Create New Procurement</h2>
                    ...
                    <div style="padding: 24px; overflow-y: auto;">
                        ...
                        <form id="createProcurementForm" action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div style="margin-bottom: 20px;">
                                <label style="display: block; font-size: 14px; font-weight: 500; color: #d1d5db; margin-bottom: 6px;">Upload Document</label>
                                <input type="file" name="document_file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" style="width: 100%; padding: 10px 12px; border: 1px solid #4b5563; background: #1f2937; color: #ffffff; border-radius: 8px; font-size: 14px;">
                                <div style="margin-top: 5px; font-size: 11px; color: #9ca3af;">Maximum file size: 20MB. Allowed: PDF, DOC, DOCX, JPG, JPEG, PNG.</div>
                            </div>

                            <input type="hidden" name="status" id="procurementStatus" value="draft">
                    ...
                    <div style="padding: 16px 24px; border-top: 1px solid #374151; background: #111827; display: flex; justify-content: flex-end; gap: 12px;" class="create-modal-actions">
                        ...
                        <button type="button" onclick="document.getElementById('procurementStatus').value='draft'; document.getElementById('createProcurementForm').submit()" style="padding: 10px 24px; border: 1px solid #4b5563; border-radius: 8px; background: #374151; color: #d1d5db; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-flex; align-items: center;" onmouseover="this.style.background='#4b5563'" onmouseout="this.style.background='#374151'">
                            <i class="fas fa-file-alt" style="margin-right: 6px;"></i> Save as Draft
                        </button>
                        <button type="button" onclick="document.getElementById('procurementStatus').value='open'; document.getElementById('createProcurementForm').submit()" style="padding: 10px 24px; background: #3b82f6; color: #ffffff; border: 1px solid #3b82f6; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all 0.2s;" onmouseover="this.style.background='#2563eb'; this.style.borderColor='#2563eb'" onmouseout="this.style.background='#3b82f6'; this.style.borderColor='#3b82f6'">
                            <i class="fas fa-paper-plane"></i> Publish Procurement
                        </button>
                    </div>
                </div>
            </div>
